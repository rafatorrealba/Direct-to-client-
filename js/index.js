...
function cantidadFacturas(){
	...
	var maxFacturas=50;
	var cantidadFact = prompt("Welcome to DTC service. How many invoices will you upload to the system? you can warranty until 50 transactions)", "1");
	//var cantidadFact = 2
	...
	if (cantidadFact == null || cantidadFact == "") {
		...
		for(i=x;i<cantidadFact;i++){
			if(x < max_fields){ //max input box allowed
				...
				$(wrapper).append('<tr id="tr-'+i+'" ><td>'+(i+1)+'</td>'+
								  ...
								  '<td><input class="filasData" style="width:150px;"  id="date-'+i+'" value="'+obtenerFechaActual()+'" type="text" name="mytext3[]"  /></td>'+
								  '<td><input class="filasData" id="total'+i+'"  type="number" name="total"/></td>'+
								  '<td><select class="filasData" id="periodo'+i+'" name="periodo"  onchange="myFunctioncambiarfeha('+i+')"><option value="0">Choose</option><option value="1">Days</option><option value="2">Weeks</option><option value="3">Months</option><option value="4">Years</option></select></td>'+
								  '<td><input class="filasData" id="fechafinal'+i+'"  style="width:150px;" type="text" name="fechafinal"/></td>'+
		...
		$(add_button).click(function(e){ //on add input button click
			...
			if(x < max_fields){ //max input box allowed
				...
				$(wrapper).append('<tr id="tr-'+x+'" ><td>'+(x+1)+'</td>'+
								  '<td><input class="filasData" id="invoice-'+x+'"  type="number" name="mytext[]"/></td>'+
				                  '<td><input class="filasData" id="serial-'+x+'"  type="number" name="mytext2[]"/></td>'+
								  '<td><input class="filasData" id="date-'+x+'" value="'+obtenerFechaActual()+'" type="text" name="mytext3[]" /></td>'+
								  '<td><input class="filasData" id="total'+x+'"  type="number" name="total"/></td>'+
								  '<td><select class="filasData" id="periodo'+x+'" name="periodo" onchange="myFunctioncambiarfeha('+x+')"><option value="0">Choose</option><option value="1">Days</option><option value="2">Weeks</option><option value="3">Months</option><option value="4">Years</option></select></td>'+
								  '<td><input class="filasData" id="fechafinal'+x+'"  style="width:150px;" type="text" name="fechafinal"/></td>'+
...
function myFunctioncambiarfeha(a){
	
	eti="total"+a;
	d = document.getElementById(eti).value;
	if( d==null || d==""){
	   alert("WARNING: you must type number of");
	   document.getElementById(eti).getElementsByTagName('option')[0].selected = 'selected'
	   return;
	}
	eti2="periodo"+a;
	c = document.getElementById(eti2).value;
	total= 1;
	
	
	eti3="date-"+a;
	var tt = document.getElementById(eti3).value;
	...
	eti2="fechafinal"+a;
	document.getElementById(eti2).value=someFormattedDate;
...
function getDataAJson(){
	...
	for (i = 0; i < elems.length; i += 4) {
		var str= elems[i+0].getAttribute('id');
		var array = str.split("-",2);
			
			
		var temp= "date-"+array[1];	
		
		var datefac= document.getElementById(temp);
		console.log(datefac.value);
		console.log(elems[i+0].value);
		console.log(elems[i+1].value);
		console.log(elems[i+2].value);
		console.log(elems[i+3].value);
		invoice=(elems[i+0].value);
		serial=(elems[i+1].value);
		date=(datefac.value);
		finaldate= (elems[i+3].value);
		
		tmp = { "class": "org.ubiot.bicicleta5.Bicycle", "GTIN": parseInt(invoice),"idInvoice": invoice, "SERIAL": parseInt(serial),"saleDate":date,"dataCreation":dataCreation,"warrantyDate":finaldate,"userCreation":userCreation,"warrantyStatus":warrantyStatus,"movementStatus": movementStatus,"manufacturing":manufacturing };
		obj.push(tmp);
	}
	todoOk=true;
	entro1vez=false;
	   ...
	   //console.log(obj[i]);
		...
		$.ajax({
			...
			error: function ajaxError(jqXHR, textStatus, errorThrown) {
				todoOk=false;
				console.error('Error requesting invoice: ', textStatus, ', Details: ', errorThrown);
				console.error('Response: ', jqXHR.responseText);
				//alert('An error occured when requesting your invoice:\n' + jqXHR.responseText);
			}
		});
	}
...
function init() {
  ...
  gapi.auth2.init({"client_id":"338660967426-bbd48sj7vocr7fh2o9spnmkkfs7dj0as.apps.googleusercontent.com"});
  console.log("cargado listo: "+gapi.auth2);
  });
}  
...
(function rideScopeWrapper($) {		//WildRydes=localStorage.getItem("WildRydes");
    ...
    WildRydes.authToken.then(function setAuthToken(token) {
		...
		//enb este punto toco quitar la seguridad pues no encontre la forma de fusionar la variable WildRydes.authToken con el inicio de sesion de gmail
        if (token) {
            authToken = token;
        } else {
    ...
    function completeRequest(result) {
		...
		FooTable.get('#table2').pageSize(15);
		//console.log(todasLasFilas);
    }
